...
const humanFavoritesController = {
  newFavorite: async (req, res) => {
    try {
      const result = await dataMapper.createFavorite(req.session.human.id, req.body.liked_profile_id); //todo  const { firstName, lastName, email, password } = req.body; this his how you do it
