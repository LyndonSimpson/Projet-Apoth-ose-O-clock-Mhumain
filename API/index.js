...
app.options("*", cors({ origin: "http://alinemassard-server.eddi.cloud", optionsSuccessStatus: 200 })); //TODO see if settings are safe

app.use(cors({ origin: "http://alinemassard-server.eddi.cloud", optionsSuccessStatus: 200  })); //TODO see if settings are safe  // app.use(cors({origin: 'localhost:3000', credentials:true })); // sinon bug coté axios ? found this all over internet : app.use(cors({origin: true, credentials: true}));
